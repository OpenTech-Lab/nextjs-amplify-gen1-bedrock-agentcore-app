...
exports.handler = awslambda.streamifyResponse(async (event, responseStream) => {
  ...
  try {
    ...
    // Support Authorization: Bearer <token> (ID token preferred for AgentCore JWT auth)
    if (!accessToken) {
      const authHeader =
    ...
    if (!accessToken) {
      console.error(
        "Missing access token (X-Access-Token or Authorization Bearer)"
      );
      ...
      responseStream.write(
        JSON.stringify({
          error:
            "Unauthorized: Missing token (provide Authorization: Bearer <token>)",
        })
      );
      responseStream.end();
      return;
